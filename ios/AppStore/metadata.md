...
24 characters. **The record is currently named "Meal Options Planner"** — rename
it under App Information before submitting, or that is the name that ships.
"Meals" is taken by someone else and can't be used.

If the rename is refused because the name is taken, fall back to
`Yet Another Meal Planner UK` or keep `Meal Options Planner`; nothing else in
this pack depends on the name.
...
| Build | 17 | The first iPhone-only build — see [ios/CHANGELOG.md](../CHANGELOG.md). Build 16 claims iPad support and would oblige you to supply 13" iPad screenshots. |
| Price | Free | |
...
No iPad set, because from build 17 the app really is iPhone-only. If App Store
