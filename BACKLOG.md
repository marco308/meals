...
- [ ] **Metrics endpoint** — phase 2 of the observability plan: `prometheus-client`, request counters/latency histograms mirroring the access log, domain counters mirroring the events, and slow-moving usage gauges (households/users/recipes) so "is anyone using this" has a graph. Guarded by a `METRICS_TOKEN` setting (404 when unset) because Traefik-style deployments route every path on the host to the API
- [ ] **Log shipping + dashboards** — phase 3: a collector tailing container logs into a store Grafana can query (Loki + Alloy + Prometheus or similar). Deployment-side plumbing: it lives with the deployment config, not in this repo
