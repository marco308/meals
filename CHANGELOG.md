...
**Two migrations.** `ef71d71574d8`, additive: four nullable columns on
`households`, and a backfill naming the lead as the payer of every household
that has paid (until now only the lead could open a checkout). Then
`67a229a2837f`, which rebuilds `household_invites` on SQLite and does nothing
on Postgres.
